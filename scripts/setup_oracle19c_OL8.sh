...
dnf update -y
dnf install bc binutils compat-libstdc++-33 dtrace-modules dtrace-modules-headers dtrace-modules-provider-headers dtrace-utils glibc libXrender libXrender-devel \
glibc-devel ksh libaio libaio-devel libX11 libXau libXi libXtst libgcc libstdc++ libstdc++-devel libxcb make nfs-utils smartmontools net-tools sysstat unzip \
...
[[ ! -d $ORACLE_BASE/oradata ]] && mkdir $ORACLE_BASE/oradata
